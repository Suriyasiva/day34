...
function Editdata() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      contextValues.editData(params.id, values);
      resetForm();
      navigate("/");
    },
  ...
  useEffect(() => {
    contextValues.singleStdData(params.id);
  }, []);
  let studentdetails = contextValues.getOneData;
  useEffect(() => {
    if (Object.keys(studentdetails).length) {
